...
class Dataset_malignacy2D_3sets(Dataset):
    def __init__(self, x_train, y_train, path_dataset, worker_init_fn=np.random.seed(0), transform = False):
        self.X = x_train
        self.y = y_train
        self.path_dataset = path_dataset
        self.transform = transform
    ...
    def __getitem__(self, idx):
        img = np.load(f'{self.path_dataset}{self.X[idx]}.npy')
...
class Dataset_orig_and_inpain_malignacy2D_3sets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = np.load(f'{self.path_masks}{self.X[idx]}.npy.npz')
        mask = mask.f.arr_0
        ...
        img = Tensor(img[None,:,:].copy()) # add the channel dimension (numChannels = 1)
        return img, target
...
def find_lr_get_losses(model_, optimizer_, criterion_, dataloader_train_):
    ...
    lr_finder = LRFinder(model_, optimizer_, criterion_, device="cuda")
...
def loss_batch_weight(model, loss_func, xb, yb, device, weights, opt=None):
    '''Based on https://pytorch.org/tutorials/beginner/nn_tutorial.html'''
    if torch.cuda.is_available():
                xb = Variable(xb.cuda(device))
                yb = Variable(yb.cuda(device))
    ...
    pred = model(xb)
    loss = loss_func(pred, yb, weight = weights)
    _, pred_class = torch.max(pred.data, 1)
    batch_total = yb.size(0)
    pred_proba = torch.softmax(pred, dim=1).detach().cpu().numpy()
    pred_proba = pred_proba[:,1]

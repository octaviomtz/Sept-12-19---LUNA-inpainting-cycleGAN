...
def around_lesion(mask_lesion, leny = 1024, lenx = 1536, lenx2 = 1248):
    switched = 0
    # "center"
    yyy, xxx = np.where(mask_lesion==True)
    yy, xx = int(np.median(yyy)), int(np.median(xxx))
    if (np.max(yyy) - np.min(yyy)) > (np.max(xxx) - np.min(xxx)):
        len_temp = leny
        leny=lenx;
        lenx=len_temp
        switched = 1
    ...
    x1 = np.max([x1,0]) 
    x2 = x1 + lenx
    return int(y1), int(y2), int(x1), int(x2), int(yy), int(xx), leny, lenx

def farthest_corner(ycen, xcen, mask_lesion):
    shapey, shapex = np.shape(mask_lesion)
    max_dist = 0; corner = 0
    dist1 = distance.cdist(np.expand_dims(np.asarray([ycen, xcen]),0),np.expand_dims(np.asarray([0,0]),0))[0][0]
    if max_dist < dist1:
        max_dist = dist1; corner = 1
    dist2 = distance.cdist(np.expand_dims(np.asarray([ycen, xcen]),0),np.expand_dims(np.asarray([0,shapex]),0))[0][0] 
    if max_dist < dist2:
        max_dist = dist2; corner = 2
    dist3 = distance.cdist(np.expand_dims(np.asarray([ycen, xcen]),0),np.expand_dims(np.asarray([shapey,0]),0))[0][0] 
    if max_dist < dist3:
        max_dist = dist3; corner = 3
    dist4 = distance.cdist(np.expand_dims(np.asarray([ycen, xcen]),0),np.expand_dims(np.asarray([shapey,shapex]),0))[0][0] 
    if max_dist < dist4:
        max_dist = dist4; corner = 4
    return corner

def image_corner(corner, leny, lenx, img, mask_lesion):
    image_too_small = False
    yyy,xxx = np.shape(mask_lesion)
    if yyy<leny or xxx<lenx:
        image_too_small = True
        img = False
        mask_lesion = False
    else:
        if corner==1:
            img=img[0:leny,0:lenx,:]
            mask_lesion=mask_lesion[0:leny,0:lenx]
        elif corner==2:
            img=img[0:leny,-lenx:,:]
            mask_lesion=mask_lesion[0:leny,-lenx:]
        elif corner==3:
            img=img[-leny:,0:lenx,:]
            mask_lesion=mask_lesion[-leny:,0:lenx]
        else:
            img=img[-leny:,-lenx:,:]
            mask_lesion=mask_lesion[-leny:,-lenx:]
    return img, mask_lesion, image_too_small
...
img = plt.imread(f'melanoma_and_skin/{filename}.jpg')
mask_lesion = np.load(f'melanoma_and_skin/mask_{filename}.npz')
